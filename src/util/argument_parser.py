...
class ArgumentParser:
    ...
    def _setup_arguments(self) -> None:
        ...
        self._parser.add_argument('-sr', '--scanner-regular', action='store_true', 
                                 help='Use regular scanner')
        self._parser.add_argument('-ss', '--scanner-stealth', action='store_true', 
                                 help='Use stealth scanner')
        self._parser.add_argument('-ip', type=str, 
                                 help='IPv4 address target')
        self._parser.add_argument('-ipv6', type=str, 
                                 help='IPv6 address target')
        self._parser.add_argument('-p', '--port', type=int, 
                                 help='Specific port to scan')
        self._parser.add_argument('-ps', '--port-start', type=int, 
                                 help='Start of port range')
        self._parser.add_argument('-pe', '--port-end', type=int, 
                                 help='End of port range')
        self._parser.add_argument('-pp', '--ping', action='store_true', 
                                 help='Enable ping')
        self._parser.add_argument('-n', '--network', type=int, 
        ...
        if args.port_start is not None and args.port_end is not None:
            if 0 <= args.port_start <= 65535 and 0 <= args.port_end <= 65535:
                if args.port_start <= args.port_end:
                    ...
                    print("Warning: Port range error: start must be less than or equal to end")
            else:
